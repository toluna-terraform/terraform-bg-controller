...
resource "aws_codebuild_project" "pr_codebuild" {
  ...
  source {
    ...
    buildspec = var.app_type != "sam" ? templatefile("${path.module}/templates/pr-created-buildspec-source.yml.tpl", { env_name = var.env_name, env_type = var.env_type, app_name = var.app_name, domain = var.domain, hosted_zone_id = data.aws_route53_zone.public.zone_id, is_managed_env = var.is_managed_env, pipeline_type = var.pipeline_type, aws_profile = var.aws_profile }) : templatefile("${path.module}/templates/pr-created-sam-buildspec-source.yml.tpl", { env_name = var.env_name, env_type = var.env_type, app_name = var.app_name, domain = var.domain, hosted_zone_id = data.aws_route53_zone.public.zone_id, is_managed_env = var.is_managed_env, pipeline_type = var.pipeline_type, aws_profile = var.aws_profile })
  }
  tags = tomap({
